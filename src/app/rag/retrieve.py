"""Retrieval: запрос -> эмбеддинг -> top-k чанков в Qdrant с score и метаданными."""
import logging
from typing import Any

from app.rag.store.qdrant_store import QdrantStore
from app.settings import Settings

log = logging.getLogger(__name__)

_settings = Settings()

_model = None


def _get_embedding_model():
    from sentence_transformers import SentenceTransformer
    return SentenceTransformer(_settings.rag_embedding_model)


def _model_singleton():
    global _model
    if _model is None:
        _model = _get_embedding_model()
    return _model


def retrieve(
    query: str,
    k: int | None = None,
    filters: dict[str, Any] | None = None,
    store: QdrantStore | None = None,
) -> list[tuple[str, float, dict[str, Any]]]:
    """
    Энкодировать запрос, поиск в Qdrant, вернуть список (chunk_id, score, meta).
    meta содержит doc_id, doc_key, title, doc_type, language, chunk_index, section, text.
    """
    if not query or not query.strip():
        log.info("[RAG] retrieve empty query -> []")
        return []
    k_val = k if k is not None else _settings.rag_default_k
    q_preview = query.strip()[:60] + "..." if len(query.strip()) > 60 else query.strip()
    log.info("[RAG] retrieve query=%r k=%s", q_preview, k_val)
    s = store if store is not None else QdrantStore()
    s.ensure_collection()
    model = _model_singleton()
    qv = model.encode([query.strip()], show_progress_bar=False).tolist()[0]
    results = s.search(qv, k=k_val, filters=filters)
    log.info("[RAG] retrieve done chunks=%d", len(results))
    return results
