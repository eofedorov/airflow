"""Индексация: загрузка документов -> sha256 -> Postgres -> чанкинг -> эмбеддинги -> Qdrant."""
import hashlib
import logging
import time
...
def run_ingestion(
    ...
    log.info("[INGESTION] done docs_indexed=%d chunks_indexed=%d duration_ms=%.2f", docs_indexed, chunks_indexed, round(elapsed_ms, 2))
    return {
